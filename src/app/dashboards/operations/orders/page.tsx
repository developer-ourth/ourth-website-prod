...
/** Visual step-tracker showing the 5-step order lifecycle */
function OrderStepTracker({ status }: { status: string }) {
  ...
  const currentIndex = STEPS.indexOf(status as OrderStep);
      ...
      {STEPS.map((step, idx) => {
        ...
        return (
          <div key={step} className="flex items-center">
            ...
            {idx > 0 && (
              <div
                className={`h-0.5 w-6 ${isPast || isCurrent ? "bg-primary" : "bg-gray-200 dark:bg-gray-700"}`}
              />
            )}
            {/* Step dot + label */}
            <div className="flex flex-col items-center gap-0.5">
              <div
                className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold transition-colors
                  ...
                  ${isFuture ? "bg-gray-100 text-gray-400 dark:bg-gray-800" : ""}
                `}
              >
                {isPast ? "✓" : idx + 1}
              </div>
              <span
                className={`hidden text-[9px] font-medium leading-tight sm:block
...
export default function OperationsOrdersPage() {
  ...
  const readyBoxCount     = orders.filter((o) => o.order_status === "processing").length;
  ...
  return (
    <DashboardGuard requiredRole="operations">
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-dark-4 dark:text-dark-6">
              Track and advance orders through each fulfilment step
        ...
        {/* Step legend */}
        <div className="flex items-center gap-3 rounded-xl border border-stroke bg-white p-4 dark:border-dark-3 dark:bg-gray-dark">
          <span className="text-xs font-semibold text-dark-4 uppercase tracking-wide shrink-0">Order Flow:</span>
          <OrderStepTracker status="pending" />
          <span className="text-dark-4 text-xs">→ confirm → ready box → dispatch → deliver</span>
        </div>
              ...
              <table className="w-full min-w-[860px]">
                <thead>
                  <tr className="border-b border-stroke bg-gray-50 dark:border-dark-3 dark:bg-gray-dark">
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase text-dark-4">Progress</th>
                    ...
                    orders.map((order) => {
                      ...
                      return (
                        <tr key={order.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-white/5">
                          ...
                          <td className="px-6 py-4">
                            <OrderStepTracker status={order.order_status} />
                          </td>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="inline-flex flex-wrap justify-end gap-2">
                              ...
                              {(order.order_status === "delivered" || order.order_status === "cancelled") && (
                                <span className={`rounded px-2 py-0.5 text-xs font-semibold capitalize ${STATUS_BADGE[order.order_status]}`}>
                                  {order.order_status === "delivered" ? "Delivered" : "Cancelled"}
                                </span>
                              )}
                            </div>
                          </td>
